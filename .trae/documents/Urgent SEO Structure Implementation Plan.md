...
- **H1 (Primary Title)**:
  ...
  - _Reasoning_: Using `sr-only` (screen reader only) ensures the H1 is present for SEO and accessibility as the "proeminent" semantic title, without breaking the existing visual design of the Hero Carousel which has its own dynamic typography. _Alternatively, if you prefer it visible, I can place it in the Hero section, but `sr-only` is the standard "urgent fix" to guaranteed structure without design regression._
